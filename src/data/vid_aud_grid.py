...
class AbstractDataset(ABC):
    def __init__(self, mode, max_v_timesteps=155, window_size=40, augmentations=False, num_mel_bins=80,
        ...
        self.f_min = 55.
        self.f_max = 7500.
        self.stft = TacotronSTFT(filter_length=640, hop_length=160, win_length=640, n_mel_channels=80,
                                 sampling_rate=16000, mel_fmin=self.f_min, mel_fmax=self.f_max)
    ...
    def build_tensor(self, frames):
        ...
        if self.augmentations:
            ...
            temporalVolume[:, :, np.maximum(0, y_s):np.minimum(112, y_s + 56),
...
class MultiDataset(Dataset, AbstractDataset):
    def __init__(self, grid, mode, max_v_timesteps=155, window_size=40, subject=None, augmentations=False,
                 num_mel_bins=80, fast_validate=False):
        assert mode in ['train', 'test', 'val']
        super().__init__(mode, max_v_timesteps, window_size, augmentations, num_mel_bins, fast_validate)
        self.grid = grid
    ...
    def __getitem__(self, idx):
        ...
        if not self.sample_window:
            ...
            audio = torch.cat([audio, torch.zeros(
                [1, int(self.max_v_timesteps / info['video_fps'] * info['audio_fps'] - aud.size(1))])], 1)
...
class LRS3Dataset(Dataset, AbstractDataset):
    def __init__(self, m_crops_path, mode, max_v_timesteps=155, window_size=40, subject=None, augmentations=False,
                 num_mel_bins=80, fast_validate=False):
        assert mode in ['trainval', 'test']
        super().__init__(mode, max_v_timesteps, window_size, augmentations, num_mel_bins, fast_validate)
        self.m_crops_path = m_crops_path
        self.file_paths = self._init_file_paths()

    def _init_file_paths(self):
        if not isinstance(self.m_crops_path, Path):
            self.m_crops_path = Path(self.m_crops_path)
        return list((self.m_crops_path / self.mode).rglob("*.npz"))
    ...
    def __getitem__(self, idx):
        ...
        ## Audio Load ##
        audio, _ = librosa.load(str(file_path).replace("mouth_crops", "audio_only")[:-3] + "wav", sr=16000)
        audio = torch.FloatTensor(audio).unsqueeze(0)
